<template>
    <AppLayout :title="'Editeaza reteta: ' + recipe.name">
        <div class="max-w-2xl mx-auto">
            <div class="flex items-center gap-3 mb-6">
                <Link :href="route('recipes.index')" class="text-gray-400 hover:text-gray-600 text-sm">← Inapoi</Link>
                <h2 class="text-xl font-semibold text-gray-800">Editeaza reteta</h2>
            </div>
...
const form = useForm({
    subject_type: props.recipe.subject_type,
    subject_id: String(props.recipe.subject_id),
    name: props.recipe.name,
    unit: props.recipe.unit,
    notes: props.recipe.notes || '',
    items: props.recipe.items.map((item) => ({
        material_id: String(item.material_id),
        quantity_per_unit: item.quantity_per_unit,
    })),
    labor_items: (props.recipe.labor_items || []).map((item) => ({
        role: item.role,
        hours_per_unit: item.hours_per_unit,
        hourly_rate: item.hourly_rate,
    })),
    equipment_items: (props.recipe.equipment_items || []).map((item) => ({
        equipment_id: String(item.equipment_id),
        hours_per_unit: item.hours_per_unit,
    })),
    drying_hours: props.recipe.drying_hours ?? '',
    curing_hours: props.recipe.curing_hours ?? '',
    wbs_stages: (props.recipe.wbs_stages || []).map((stage) => ({
        name: stage.name,
...
function remove() {
    if (confirm(`Stergi reteta "${props.recipe.name}"?`)) {
