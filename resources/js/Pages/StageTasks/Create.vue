<template>
    <AppLayout title="Task etapa nou">
        <div class="max-w-3xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h2 class="text-xl font-semibold text-gray-800">Adauga task pe etapa</h2>
                    <p class="text-sm text-gray-500 mt-1">Planifica actiunile operationale pentru executie.</p>
                </div>
            ...
            <form @submit.prevent="submit" class="bg-white border border-gray-200 rounded-xl p-6 space-y-4">
                ...
                <div class="flex gap-2 pt-2">
                    <button type="submit" :disabled="form.processing" class="bg-orange-500 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-orange-600 disabled:opacity-50">
                        {{ form.processing ? 'Se salveaza...' : 'Salveaza task' }}
                    </button>
...
function submit() {
    form.post(route('stage-tasks.store'));
}
</script>
